...
int main(void)
{
    ...
    switch (childList[currChildQty].pid = fork()) 
    {
        ...
        case 0:
            ...
            my_pid = getpid();
            printf("\x1B[33m SERVER (%d)\x1B[0m: Im the new child.\n", my_pid);
            main_parent_server();
            break;
        ...
        default:
            ...
            printf("\x1B[32m PRODUCER (%d)\x1B[0m: Starting producers routine..\n", my_pid);
            currChildQty++;
            print_child_table();
...
void main_parent_producer (void)
{
    ...
    ******************************************************************************/
    ...
    key_t key;
    data_t buffer[SHM_SIZE];          // Buffer that we will read
    ...
    if((void*) (shm_data = shmat(shmid, NULL, 0)) == NULL) // Connects the shm to the task
    ...
    while(1)
    {
        ...
        while ( buffCnt < SHM_SIZE)
        {
            while((bytesRead = read_sensor(&(buffer[buffCnt]), configs.coefFilter)) <= 0)
                usleep(10 * 1000); // Retries on error (10ms)
...
void main_parent_server (void)
{
    ...
    ******************************************************************************/
    ...
    while (1) 
    {
        ...
        if(socketChild < 0 )
        {
            printf("\x1B[33m SERVER (%d)\x1B[0m: Could not accept an incomming connection. Skipping..", my_pid);
            continue;
        }
        ...
        switch (childList[currChildQty].pid = fork()) 
        {
            ...
            case 0:
                ...
                my_pid = getpid();
                currChildQty = 0;
                printf("\x1B[36m CHILD (%d)\x1B[0m: Im the new child.\n", my_pid);
                printf("\x1B[36m CHILD (%d)\x1B[0m: New client was accepted: %s \n", my_pid, inet_ntoa(cli_addr.sin_addr));
            ...
            default:
                // Parent routine
                printf("\x1B[33m SERVER (%d)\x1B[0m: \x1B[36m CHILD (%d)\x1B[0m accepted one connection from IP (%s)\n", my_pid, childList[currChildQty].pid, inet_ntoa(cli_addr.sin_addr));
                currChildQty++;
                close(socketChild);
                print_child_table();
                break;
...
void main_child(int socketChild)
{
    ...
    ******************************************************************************/
            ...
            case CHILD_FSM_ENVIANDO:
                if (!sem_libre || buffer_index >= SHM_SIZE)
                {
                    ...
                    if(word[0]=='K' && word[1]=='A')
                    {
                        ...
                        if ( ( numbytes = send(socketChild, &(shm_data[buffer_index]), sizeof(data_t), 0) ) < 0) 
                            exit_on_error("send data port");

                        if (numbytes == sizeof(data_t))
                        {
                            buffer_index += 1;
                        }else{
                            printf("\x1B[36m CHILD (%d)\x1B[0m: ERROR - Se enviaron menos datos de los adecuados!\n", my_pid);
                            clean_and_exit(0);
                        }
    ...
    // Should never get here.
    printf("\x1B[36m CHILD (%d)\x1B[0m: Exits before ACK was received.\n", my_pid);
    clean_and_exit(0);
}
...
void sigchld_handler(int s)
{
    ...
    ******************************************************************************/
    ...
    do
    {
        ...
        printf("\x1B[37m PARENT (%d)\x1B[0m: \x1B[36m CHILD (%d)\x1B[0m [status: 0x%2.2X] was killed.\n", my_pid, child_pid, WTERMSIG(status));
...
void sigpipe_handler(int s)
{
    ...
    ******************************************************************************/
    ...
    printf("\n\n\x1B[36m CHILD (%d)\x1B[0m: Broken PIPE. Exiting..\n\n", my_pid);
    exit_child(0);
    return;
}
...
void sigusr2_handler(int s)
{
    ...
    {
        printf("\x1B[32m PRODUCER (%d)\x1B[0m: Sending SIGUSR2 to server..\n", my_pid);
        ...
        printf("\x1B[32m PRODUCER (%d)\x1B[0m: New filter coefficients:\n", my_pid);
        for (int kk = 0; kk < MAX_COEF; kk++)
            printf("coef[%d]: %f\n", kk, configs.coefFilter[kk]);
    }else if (clean_and_exit == exit_server)
    {   
        reload(&configs);
        printf("\x1B[33m SERVER (%d)\x1B[0m: New max connection limit was stablished to: %d\n", my_pid, configs.maxConnections);
...
void print_child_table(void)
{
    ...
    ******************************************************************************/
    if (clean_and_exit == exit_producer)
    {
        printf("---------- \x1B[32m PRODUCER (%d)\x1B[0m -------------\n", my_pid);
    }else if (clean_and_exit == exit_server)
    {
        printf("----------- \x1B[33m SERVER (%d)\x1B[0m --------------\n", my_pid);
    }else{
        printf("----------- \x1B[37m PARENT (%d)\x1B[0m --------------\n", my_pid);
    }
    ...
    ******************************************************************************/
    ...
    if((fp = fopen(PATH_CONFIG,"r"))){
        while(!feof(fp))
        {
            ...
            filtroStr = fgets(linea, sizeof(linea), fp);
